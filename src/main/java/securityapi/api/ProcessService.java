            ...
            if ("new".equals(infoType)) {
                ...
                for (Map.Entry<String, String> entry : originalDataMap.entrySet()) {
                    ...
                    if (currentColumn.equals(passwordColumn)) {
                        ...
                        dataToInsert.put(currentColumn, encryptionService.encrypt(currentValue, iv));
                    } else {
                        dataToInsert.put(currentColumn, currentValue);
                    }
                }

                dbManager.insertNewData(conn, table, dataToInsert, columnsToEncrypt, iv, algo);
                response.put("status", "success");
                response.put("message", "새로운 데이터 처리 및 DB 저장 완료.");
                response.put("result_preview", dataToInsert);
                ...
                if (!requestedColumns.isEmpty()) {
                    ...
                    for (String col : columnsToProcess) {
                        ...
                        if (value != null) {
                            ...
                            String result = isEncryptMode ? encryptionService.encrypt(value, ivToUse) : encryptionService.decrypt(value, ivToUse);
                            processedData.put(col, result);
                        }
                    }
                }
                
                if ("T".equalsIgnoreCase(updateFlag)) {
