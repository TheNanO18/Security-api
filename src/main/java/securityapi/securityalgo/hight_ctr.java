...
// 클래스 이름을 hight_ctr로 변경하고, Algorithm 인터페이스를 구현합니다.
public class hight_ctr implements algorithm {
    ...
    public static byte[] HIGHT_CTR_Encrypt_Internal( byte[] pbszUserKey, byte[] pbszCTR, byte[] message, int message_offset, int message_length ) {
        int nOutLeng[] = { 0 };
        ...
        byte[] pbszOutputText = new byte[message_length + nInputTextPadding]; // 출력 버퍼 크기 조정
        ...
        data = chartoint32_for_HIGHT_CTR(newpbszInputText, newpbszInputText.length); // 패딩된 길이 사용

        HIGHT_CTR_Process( info, data, newpbszInputText.length, outbuf, nOutLeng ); // 패딩된 길이 사용
        ...
        // 패딩된 길이 그대로 반환 (CTR 모드에서는 패딩은 개념적으로 XOR 스트림 길이에 맞추기 위한 것이므로 결과 길이는 블록 단위로 맞춰짐)
        // 실제 유효한 데이터 길이는 message_length 입니다.
        byte[] finalResult = new byte[nOutLeng[0] - nPaddingLeng[0]];
        System.arraycopy(pbszOutputText, 0, finalResult, 0, finalResult.length);
        return finalResult;
    }


    public static byte[] HIGHT_CTR_Decrypt_Internal( byte[] pbszUserKey, byte[] pbszCTR, byte[] message, int message_offset, int message_length ) {
        int nOutLeng[] = { 0 };
        int nPaddingLeng[] = new int[] { 0 };
        KISA_HIGHT_INFO info = new KISA_HIGHT_INFO();
        int[] outbuf;
        ...
        HIGHT_CTR_init( info, KISA_ENC_DEC.KISA_ENCRYPT, pbszUserKey, pbszCTR ); // Note: CTR mode encrypt/decrypt use the same encryption function
    ...
    public static final class KISA_HIGHT_INFO {
        ...
        public int cbc_buffer[] = new int[2]; // Note: This field name 'cbc_buffer' might be misleading for CTR mode, but keeping it as is from original code.
        public int buffer_length;
        public int[] cbc_last_block = new int[2]; // Note: This field name 'cbc_last_block' might be misleading for CTR mode, but keeping it as is from original code.
    ...
    // `Algorithm` 인터페이스의 메서드를 구현합니다.
    @Override
    public byte[] process(byte[] data, byte[] key, byte[] ivOrCounter, boolean encryptMode) {
        System.out.println("hight_ctr (통합된) 알고리즘으로 데이터를 처리합니다.");
        ...
        if (encryptMode) {
            // 암호화 수행
            System.out.println("--- 암호화 시작 (CTR 모드) ---");
            result = HIGHT_CTR_Encrypt_Internal(key, ivOrCounter, data, 0, data.length);
            ...
            // 복호화 수행 (CTR 모드는 암호화/복호화에 동일한 HIGHT_CTR_Encrypt 함수를 사용)
            System.out.println("--- 복호화 시작 (CTR 모드) ---");
            result = HIGHT_CTR_Decrypt_Internal(key, ivOrCounter, data, 0, data.length); // HIGHT_CTR_Decrypt_Internal 호출
            ...
            if (result != null) {
                ...
                System.out.println();
                System.out.println("복호화된 데이터 (String): " + new String(result)); // CTR은 패딩 없으므로 trim() 불필요
            } else {
    ...
    public String getName() {
        return "hight_ctr"; // 클래스 이름과 동일하게 반환
    }

    // 원본 main 메서드는 이 통합된 클래스에서는 제거합니다.
