// Ariatest/hight_cbc.java
package securityapi.securityalgo;

/**
@file KISA_HIGHT_CBC.java
@brief HIGHT CBC 암호 알고리즘 Wrapper
@author Copyright (c) 2013 by KISA
@remarks http://seed.kisa.or.kr/
*/

// 클래스 이름을 hight_cbc로 정의하고, Algorithm 인터페이스를 구현합니다.
public class hight_cbc implements algorithm {
    ...
    public static byte[] HIGHT_CBC_Encrypt_Internal( byte[] pbszUserKey, byte[] pszbIV, byte[] message, int message_offset, int message_length )
    {
        ...
        data = chartoint32_for_HIGHT_CBC(newpbszPlainText, nPlainTextLen); // Use nPlainTextLen here, not newpbszPlainText.length

        HIGHT_CBC_Process( info, data, nPlainTextLen, outbuf, nRetOutLeng ); // Use nPlainTextLen
    ...
    public static byte[] HIGHT_CBC_Decrypt_Internal( byte[] pbszUserKey, byte[] pszbIV, byte[] message, int message_offset, int message_length )
    {
        KISA_HIGHT_INFO info = new KISA_HIGHT_INFO();
        int[] outbuf;
        int[] data;
        ...
        byte []newpbszCipherText = new byte[nCipherTextLen]; // This is the duplicate variable.
    ...
    // ... (rest of the internal classes and original main method from KISA_HIGHT_CBC)
    // Note: The original main method and Common class should be removed if this file is becoming the Algorithm wrapper.
    // However, per the user's last explicit request, this is just the raw code with the error noted.
    // The previous request was to make it *selectable* as one, implying this file *becomes* the wrapper.
    ...
    public static void main(String[] args)
    {
        ...
        byte[] outary =  HIGHT_CBC_Encrypt_Internal(  pbUserKey,  bszIV,  pbData, 0, 1 );
        byte[] pbPlains = HIGHT_CBC_Decrypt_Internal(pbUserKey,  bszIV,  outary, 0, 8 ) ;        
        
        
        int temp_num=1;
        for(int k=0; k<7; k++)
        {
            outary =  HIGHT_CBC_Encrypt_Internal(  pbUserKey,  bszIV,  pbData, 0,   temp_num );
            pbPlains = HIGHT_CBC_Decrypt_Internal(pbUserKey,  bszIV,  outary,0,  8 ) ;    
        ...
        {
            outary =  HIGHT_CBC_Encrypt_Internal(  pbUserKey,  bszIV,  pbData,0,  temp_num );
            pbPlains = HIGHT_CBC_Decrypt_Internal(pbUserKey,  bszIV,  outary,0,  16 ) ;    
        ...
        byte[] outary1 =  HIGHT_CBC_Encrypt_Internal(  pbUserKey,  bszIV,  pbData1,0,  2 );
        byte[] pbPlains1 = HIGHT_CBC_Decrypt_Internal(pbUserKey,  bszIV,  outary1, 0, 8 ) ;           
        ...
        int PLAINTEXT_LENGTH_CASE2 = 2; // Renamed to avoid duplicate local variable
        
        int CIPHERTEXT_LENGTH_CASE2 = 8; // Renamed to avoid duplicate local variable
        
        System.out.print("\n\n[ Test Encrypt mode :  Case 2 ]"+"\n");
        System.out.print("Key                               : ");
        for (int i=0; i<16; i++)    System.out.print(Integer.toHexString(0xff&pbUserKey[i])+", ");
        System.out.print("\n");
        System.out.print("Plaintext                         : ");
        for (int i=0; i<PLAINTEXT_LENGTH_CASE2; i++)  System.out.print(Integer.toHexString(0xff&pbData[i])+", ");
        System.out.print("\n");
        ...
        PLAINTEXT_LENGTH_CASE2 = 2;
        
        CIPHERTEXT_LENGTH_CASE2 = 8;
        ...
        int nPlainTextPadding = BLOCK_SIZE_HIGHT - (PLAINTEXT_LENGTH_CASE2 % BLOCK_SIZE_HIGHT);
        byte []newpbszPlainText_Case2 = new byte[PLAINTEXT_LENGTH_CASE2+nPlainTextPadding]; // Renamed
        Common.arraycopy(newpbszPlainText_Case2, pbData, PLAINTEXT_LENGTH_CASE2);       
        
        
        byte[] pbszCipherText_Case2 = new byte[newpbszPlainText_Case2.length];           // Renamed
        

        HIGHT_CBC_init( info, KISA_ENC_DEC.KISA_ENCRYPT, pbUserKey, bszIV );
        
        int process_blockLeng = BLOCK_SIZE_HIGHT * 2;                               // 한 번에 처리할 BLOCK

        int[] outbuf = new int[process_blockLeng/4];
        
        for(j=0; j<PLAINTEXT_LENGTH_CASE2-process_blockLeng; )
        {           
            System.arraycopy(pbData, j, newpbszPlainText_Case2, 0, process_blockLeng); // Use renamed variable
            data = chartoint32_for_HIGHT_CBC(newpbszPlainText_Case2, process_blockLeng);          // Use renamed variable
            HIGHT_CBC_Process( info, data, process_blockLeng, outbuf, nRetOutLeng );            
            cdata = int32tochar_for_HIGHT_CBC(outbuf, nRetOutLeng[0]);            
            System.arraycopy(cdata, 0, pbszCipherText_Case2, j, nRetOutLeng[0]);          // Use renamed variable
            j += nRetOutLeng[0];
        }
        
        int remainleng = PLAINTEXT_LENGTH_CASE2 % process_blockLeng;
        if(remainleng == 0)
        {
            remainleng = process_blockLeng;
        }
        System.arraycopy(pbData, j, newpbszPlainText_Case2, 0, remainleng); // Use renamed variable
        data = chartoint32_for_HIGHT_CBC(newpbszPlainText_Case2, remainleng); // Use renamed variable
        ...
        System.arraycopy(cdata, 0, pbszCipherText_Case2, j, nPaddingLeng[0]); // Use renamed variable
        ...
        for (int i=0; i<CIPHERTEXT_LENGTH_CASE2; i++)
            System.out.print(Integer.toHexString(0xff&pbszCipherText_Case2[i])+", "); // Use renamed variable
        System.out.print("\n");
        ...
        byte[] pbszEncryptedMessage = new byte[CIPHERTEXT_LENGTH_CASE2];
        System.arraycopy(pbszCipherText_Case2, pbszCipherText_offset, pbszEncryptedMessage, 0, CIPHERTEXT_LENGTH_CASE2); // Use renamed source
        ...
        byte []newpbszCipherText_inner = new byte[nCipherTextLen]; // Renamed to avoid duplicate
        byte []pbszPlainText = new byte[nCipherTextLen];
        
        
        for(j=0; j<nCipherTextLen - process_blockLeng; )
        {
            System.arraycopy(pbszEncryptedMessage, j, newpbszCipherText_inner, 0, process_blockLeng); // Use renamed variable
            data = chartoint32_for_HIGHT_CBC(newpbszCipherText_inner, process_blockLeng); 
            HIGHT_CBC_Process( info, data, process_blockLeng, outbuf, nRetOutLeng );            
            cdata = int32tochar_for_HIGHT_CBC(outbuf, nRetOutLeng[0]);            
        ...
        System.arraycopy(pbszEncryptedMessage, j, newpbszCipherText_inner, 0, remainleng); // Use renamed variable
        data = chartoint32_for_HIGHT_CBC(newpbszCipherText_inner, remainleng);    
        HIGHT_CBC_Process( info, data, remainleng, outbuf, nRetOutLeng );            
        cdata = int32tochar_for_HIGHT_CBC(outbuf, nRetOutLeng[0]);            
        System.arraycopy(cdata, 0, pbszPlainText, j, nRetOutLeng[0]);            
        j += nRetOutLeng[0];
        ...
        for (int i=0; i<PLAINTEXT_LENGTH_CASE2; i++)
        ...
        nPlainTextPadding = BLOCK_SIZE_HIGHT - (PLAINTEXT_LENGTH_CASE2 % BLOCK_SIZE_HIGHT);
        newpbszPlainText_Case2 = new byte[PLAINTEXT_LENGTH_CASE2+nPlainTextPadding]; // Renamed
        Common.arraycopy(newpbszPlainText_Case2, pbData, PLAINTEXT_LENGTH_CASE2);       
        
        
        pbszCipherText_Case2 = new byte[newpbszPlainText_Case2.length];           // Renamed
        

        HIGHT_CBC_init( info, KISA_ENC_DEC.KISA_ENCRYPT, pbUserKey, bszIV );
        
        process_blockLeng = BLOCK_SIZE_HIGHT * 2;                               // 한 번에 처리할 BLOCK
        ...
        remainleng = PLAINTEXT_LENGTH_CASE2 % process_blockLeng;
        if(remainleng == 0)
        {
            remainleng = process_blockLeng;
        }
        ...
        pbszEncryptedMessage = new byte[CIPHERTEXT_LENGTH_CASE2];
        System.arraycopy(pbszCipherText_Case2, pbszCipherText_offset, pbszEncryptedMessage, 0, CIPHERTEXT_LENGTH_CASE2); // Use renamed source
        ...
        newpbszCipherText_inner = new byte[nCipherTextLen]; // Renamed to avoid duplicate
    ...
    @Override
    public byte[] process(byte[] data, byte[] key, byte[] ivOrCounter, boolean encryptMode) {
        System.out.println("hight_cbc (통합된) 알고리즘으로 데이터를 처리합니다.");

        System.out.print("원본 데이터 (byte[]): ");
        for (byte b : data) {
            System.out.printf("%02x ", b);
        }
        System.out.println();
        System.out.println("데이터 길이: " + data.length + " 바이트");

        byte[] result = null;

        if (encryptMode) {
            // 암호화 수행
            System.out.println("--- 암호화 시작 (CBC 모드) ---");
            // KISA_HIGHT_CBC.HIGHT_CBC_Encrypt 대신 내부 static 메서드 HIGHT_CBC_Encrypt_Internal 호출
            result = HIGHT_CBC_Encrypt_Internal(key, ivOrCounter, data, 0, data.length);
            System.out.print("암호화된 데이터 (hex): ");
            if (result != null) {
                for (byte b : result) {
                    System.out.printf("%02x ", b);
                }
            ...
            // 복호화 수행
            System.out.println("--- 복호화 시작 (CBC 모드) ---");
            // KISA_HIGHT_CBC.HIGHT_CBC_Decrypt 대신 내부 static 메서드 HIGHT_CBC_Decrypt_Internal 호출
            result = HIGHT_CBC_Decrypt_Internal(key, ivOrCounter, data, 0, data.length);
            System.out.print("복호화된 데이터 (hex): ");
            if (result != null) {
                for (byte b : result) {
                    System.out.printf("%02x ", b);
                }
                System.out.println();
                // CBC는 패딩이 있으므로, 원본 길이를 알거나 패딩 제거 로직이 필요할 수 있습니다.
                // 여기서는 간단히 String으로 변환 후 trim() 처리
                System.out.println("복호화된 데이터 (String): " + new String(result).trim());
            } else {
    ...
    // Algorithm 인터페이스의 getName 메서드를 구현합니다.
    @Override
    public String getName() {
        return "hight_cbc"; // 이 클래스의 이름을 반환합니다.
    }
